    ...
    networking = {
      ...
      wg-quick.interfaces = {
        ...
        wg-link = {
          ...
          postUp = ''
            ${pkgs.iptables}/bin/iptables -A FORWARD -i wg-link -j ACCEPT
            ${pkgs.iptables}/bin/iptables -t nat -A POSTROUTING -s 10.5.5.1/24 -o eth0 -j MASQUERADE
            ${pkgs.iptables}/bin/ip6tables -A FORWARD -i wg-link -j ACCEPT
            ${pkgs.iptables}/bin/ip6tables -t nat -A POSTROUTING -s fdc9:281f:04d7:9ee9::1/64 -o eth0 -j MASQUERADE
          '';
          # Undo the above
          preDown = ''
            ${pkgs.iptables}/bin/iptables -D FORWARD -i wg-link -j ACCEPT
            ${pkgs.iptables}/bin/iptables -t nat -D POSTROUTING -s 10.5.5.1/24 -o eth0 -j MASQUERADE
            ${pkgs.iptables}/bin/ip6tables -D FORWARD -i wg-link -j ACCEPT
